...
public class KMeansRun {
    ...
    private List<float[]> originalData = null;    //用于存放，原始数据集
    ...
    public KMeansRun(int k, List<float[]> originalData, List<Integer> idList) {

        this.kNum = k; // 初始化簇的个数
        this.originalData = originalData; // 初始化数据
        this.len = originalData.get(0).length;  // 初始化维度数
    ...
    private void init() {
        ...
        for (int i = 0, j = originalData.size(); i < j; i++) {
            ...
            Point point = new Point(idList.get(i), originalData.get(i));
    ...
    public void cluster(Set<Cluster> clusterSet) {
        /**
         *  1 首先进行一个二重的循环，用每个点去遍历中心点，得到每个点距离最近的作为中心点
         */
