...
//		}
        ...
        DataStream<TerminalInfo> useTypeMap = input.flatMap(new TerminalFlatMap());


        DataStream<TerminalInfo> useTypeReduce = useTypeMap
                .keyBy("groupField")
                .timeWindowAll(Time.seconds(2))
                .reduce(new TerminalReduce());

        useTypeReduce.addSink(new TerminalSink());
